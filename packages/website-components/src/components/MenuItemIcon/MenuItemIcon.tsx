...
export const MenuItemIcon = ({ children, open, ...props }: MenuItemIconProps) => {
  ...
  return (
    <StyledStack {...props} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} open={open}>
      <StyledTypography open={open} variant="buttonSmall">
        {children}
      </StyledTypography>
      <StyledIconWrapper>
        <StyledIcon src={arrowDownIdle} alt="" sx={isHovered ? {} : { opacity: 1 }} open={open} />
        <StyledIcon src={arrowDownHover} alt="" sx={isHovered ? { opacity: 1 } : {}} open={open} />
