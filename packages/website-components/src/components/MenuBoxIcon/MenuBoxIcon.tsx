...
export const MenuBoxIcon: React.FC<MenuBoxIconProps> = ({ iconIdle, iconHover, headline, text, link, ...props }) => {
  ...
  return (
    <Link to={link}>
      <StyledStack onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} {...props}>
        <Hidden mdDown>
          <StyledImageWrapper sx={{ position: 'relative' }}>
            <StyledImage src={iconHover} alt="" sx={isHovered ? { opacity: 1 } : {}} />
            <StyledImage src={iconIdle} alt="" sx={isHovered ? {} : { opacity: 1 }} />
          </StyledImageWrapper>
        </Hidden>
        <StyledTextWrapper>
