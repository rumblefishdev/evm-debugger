...
export const MenuItemIconLink = ({ to, children, noIcon, ...props }: MenuItemIconLinkProps) => {
  ...
  return (
    <Link to={to}>
      <StyledStack {...props} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}>
        <StyledTypography variant="buttonSmall">{children}</StyledTypography>
        <StyledIconWrapper>
          {!noIcon && (
            <>
              <StyledIcon src={arrowDownHover} alt="" sx={isHover ? { opacity: 1 } : {}} />
              <StyledIcon src={arrowDownIdle} alt="" sx={isHover ? {} : { opacity: 1 }} />
            </>
          )}
        </StyledIconWrapper>
      </StyledStack>
