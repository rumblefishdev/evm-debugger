...
export const getFilteredStructLogs = (
...
export const checkIfOfCallType = (
  item: TReturnedTraceLog | IStructLog,
): item is ICallTypeTraceLog | ICallTypeStructLogs => {
  if ('type' in item)
    return (
      item.type === 'CALL' ||
      item.type === 'CALLCODE' ||
      item.type === 'DELEGATECALL' ||
...
export const checkIfOfCreateOrCallType = (
  item: TReturnedTraceLog,
): item is TMainTraceLogs => {
...
export const checkIfOfReturnType = (
  item: TReturnedTraceLog | IFilteredStructLog,
): item is IReturnTypeTraceLog | IReturnTypeStructLogs => {
...
export const getSafeHex = (value: string | undefined) =>
  value ? `0x${value}` : `0x`
...
export const prepareTraceToSearch = (
  ...
  maxLastCallIndex = inclusiveLastElement
    ? maxLastCallIndex + 1
    : maxLastCallIndex
  return maxLastCallIndex === -1
    ? slicedTraceLogs
    : slicedTraceLogs.slice(0, maxLastCallIndex)
}

export const getNextItemOnSameDepth = (
  traceLogs: IStructLog[],
  currentIndex: number,
  depth: number,
): IStructLog => {
  const traceToSearch = prepareTraceToSearch(
    traceLogs,
    currentIndex,
    depth,
    true,
...
export const getLastItemInCallTypeContext = (
  ...
  return traceToSearch.find(
    (iteratedItem) =>
      ...
      (iteratedItem.type === 'RETURN' ||
        iteratedItem.type === 'REVERT' ||
        iteratedItem.type === 'STOP'),
  ) as IReturnTypeTraceLog | IStopTypeTraceLog
}

export const getLastLogWithRevertType = (
  traceToSearch: TReturnedTraceLog[],
  depth: number,
) => {
  return traceToSearch.find(
    (iteratedItem) =>
      iteratedItem.depth === depth + 1 && iteratedItem.type === 'REVERT',
