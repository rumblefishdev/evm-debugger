...
export const newConversion = (functionStack: TContractFunction[], rootDepth: number): TNestedFunction => {
  ...
  return {
    innerFunctions: innerFunctions
      .map((func) => newConversion(functionStack.slice(functionStack.indexOf(func)), func.depth))
