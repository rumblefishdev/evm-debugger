...
import { createBaseSettingsObject } from '../utils'
...
class MultiFileExtendedSourceManager {
  ...
  public extractFiles(sourceData: TEtherscanContractSourceCodeResult): TExtractedSourceFiles {
    const rawSourceCode = sourceData.SourceCode.replace(/(\r\n)/gm, '').slice(1, -1)

    const sourceCodeObj: TEtherscanParsedSourceCode = JSON.parse(rawSourceCode)

    return Object.keys(sourceCodeObj.sources).map((fileName) => [fileName, sourceCodeObj.sources[fileName].content])
  }

  public createSettingsObject(sourceData: TEtherscanContractSourceCodeResult): TSolcConfiguration {
    const rawSourceCode = sourceData.SourceCode.replace(/(\r\n)/gm, '').slice(1, -1)
    const sourceCodeObj: TEtherscanParsedSourceCode = JSON.parse(rawSourceCode)
...
class MultiFileSourceManager {
  public static isApplicable(sourceCode: string): boolean {
    ...
    return hasMoreThanZeroContent && !MultiFileExtendedSourceManager.isApplicable(sourceCode)
  }
  public extractFiles(sourceData: TEtherscanContractSourceCodeResult): TExtractedSourceFiles {
    const rawSourceCode = sourceData.SourceCode.replace(/(\r\n)/gm, '')

    const sourceCodeObj: Record<string, { content: string }> = JSON.parse(rawSourceCode)

    return Object.keys(sourceCodeObj).map((fileName) => [fileName, sourceCodeObj[fileName].content])
  }
