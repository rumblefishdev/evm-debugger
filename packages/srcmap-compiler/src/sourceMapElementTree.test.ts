import {
  SourceMapElementTreeNode,
  SourceMapElementTree,
} from './sourceMapElementTree'
...
describe('SourceMapElementTreeNode', () => {
  describe('addChild', () => {
    ...
    beforeEach(() => {
      ...
      nodes = els.map((element, index) => new SourceMapElementTreeNode(element))
    })
...
describe('SourceMapElementTree', () => {
  ...
  beforeEach(() => {
    ...
    const opCodes = ['ADD', 'JUMP', 'GASLIMIT', 'PUSH20', 'STOP', 'POP']

    const context = new SourceMapContext(sourceCode, opCodes)
  ...
  describe('addNewNode', () => {
    it('can construct linear tree', () => {
      const tree = new SourceMapElementTree(nodes[0])
    ...
    it('will only add id for the same node', () => {
      const tree = new SourceMapElementTree(nodes[0])
    ...
    it('can handle two roots', () => {
      const tree = new SourceMapElementTree(nodes[1])
  ...
  describe('shrinkTree', () => {
    it('works', () => {
      ...
      for (const node of [nodes[1], nodes[2], nodes[5]]) tree.addNewNode(node)
      ...
      expect(shrunkTree.rootNode.children[0].uniqueId).toEqual(
        nodes[1].uniqueId,
      )
      expect(shrunkTree.rootNode.children[0].children).toHaveLength(1)
      expect(shrunkTree.rootNode.children[0].children[0].uniqueId).toEqual(
        nodes[2].uniqueId,
      )
      expect(shrunkTree.rootNode.children[0].children[0].children).toHaveLength(
        0,
      )
    })
  })
