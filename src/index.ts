...
async function main() {
    ...
    for (let i = 0; i < extractedDataFromTraceLogs.length; i++) {
        const item = extractedDataFromTraceLogs[i]

        if (chceckIfOfCallType(item)) {
            const isContract = await traceLogsParserHelper.checkIfAddressIsContract(item.address)
            extractedDataFromTraceLogs[i] = { ...item, isContract }
        }
    }
